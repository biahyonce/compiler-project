package com.github.biancacristina.compiler.syntax.grammar.request;

import java.util.Arrays;
import java.util.HashMap;
import java.util.List;

public class FirstRequest {
    private String production;
    private HashMap<String, Boolean> first;

    public FirstRequest(String[] request) {
        List<String> requestList = Arrays.asList(request);
        this.production = requestList.get(0);
        this.first = new HashMap<>();
        requestList.subList(1, requestList.size()).forEach( token -> first.put(token, Boolean.TRUE) );
    }

    public String getProduction() {
        return production;
    }

    public HashMap<String, Boolean> getFirst() {
        return first;
    }
}
